  ...
  if (error || !product) {
    return (
      <div className="p-8 text-center">
        <p className="mb-2 text-gray-700">{error || 'Product not found.'}</p>
  ...
  return (
    <div className="mx-auto max-w-4xl p-4 sm:p-8">
      ...
      <nav className="mb-4 flex flex-wrap items-center gap-1.5 text-sm text-gray-500">
        ...
        <span>/</span>
        <span className="max-w-48 truncate font-medium text-gray-700">{product.name}</span>
      </nav>
              ...
              {galleryImages.map((img, idx) => (
                <button
                  ...
                  className={`h-16 w-16 shrink-0 overflow-hidden rounded-md border-2 transition-colors ${
                    idx === activeImageIdx ? 'border-orange-500' : 'border-transparent hover:border-gray-300'
                  }`}
                >
      ...
      <div className="flex flex-1 flex-col gap-3">
        <div className="flex items-start justify-between gap-3">
          ...
          <button
            ...
            className="mt-1 shrink-0 rounded-full border border-gray-200 p-2 transition-transform hover:scale-110 hover:bg-gray-50"
          >
            <svg
              ...
              stroke="currentColor"
              strokeWidth={1.8}
              className={`h-6 w-6 transition-colors ${isWished(product.id) ? 'text-orange-500' : 'text-gray-500'}`}
            >
        ...
        {selectedVariant && selectedVariant.stock > 0 && (
          <div>
            ...
            <div className="mt-1 inline-flex items-center rounded-md border border-gray-300">
              <button
                onClick={() => setQuantity((q) => Math.max(1, q - 1))}
                disabled={quantity <= 1}
                aria-label="Decrease quantity"
                className="px-3 py-1.5 text-gray-600 transition-colors hover:bg-gray-100 disabled:cursor-not-allowed disabled:opacity-40"
              >
                −
              </button>
